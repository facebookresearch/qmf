...
Our results are captured on a Jetson Orin Nano 8GB, which provides a 6-core Arm Cortex-A78AE, and a 1024 CUDA-core GPU.   We run the Jetson in \SI{15}{\watt} mode. For our CPU results, we use only a single CPU core for runtime animation.  We demonstrate our technique with 6 blendshape datasets.  \Table{tab:FullResults} and \Figure{fig:Datasets} show our datasets.  Aura, Bowen, Jupiter, and Proteus are publicly available models \cite{kavan2024compressed}.  Louise is the Animatable Digital Double of Louise by Eisko© ( \href{http://www.eisko.com}{http://www.eisko.com} ).  We have also created a new face model, Carlos.
...
We first ran experiments to evaluate the effect of our new loss function (\Eq{eq:NewLoss}) on \csfb. It led to significant quality improvements in wrinkle areas (see \Figure{fig:CSFBAura}) and slight metric gains (see \Table{tab:FullResults}). Having established its effectiveness, all further comparisons with our sparse matrix factorization use \csfb with the new loss.
\begin{figure}[t]
...
$\alpha$ increased, the noise metric improved rapidly, while the position metrics degraded almost linearly. % (see \Figure{fig:chooseAlpha})
 For every model we choose the biggest $\alpha$ value that did not significantly degrade the position metrics.
...
As shown in \replace{\Figure{fig:Teaser} and \Figure{fig:QMFNewNormAndWrinkles}}{Figures \ref{fig:Teaser}, \ref{fig:QMFNewNormAndWrinkles}, and \ref{fig:poses}}, the sparse factorization method preserves details in challenging wrinkle areas. 
%
...
Detailed results for all experiments are shown in \Table{tab:FullResults}.   \replaceII{Full optimization code for our method will be released open source concurrent to the conference.}{We have made our sparse factorization optimization code available at www.dummy.url}  See the supplemental video to view runtime results and the supplemental PDF for CPU code.
%We have open-sourced our codebase at
