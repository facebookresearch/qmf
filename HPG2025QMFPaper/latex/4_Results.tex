...
In our experiments, we use traditional sparse blendshapes as a baseline.  As part of our preprocessing pipeline, deltas smaller than $10 \mu m$ in absolute value are clamped to zero. 
...
We compare our method against \csfb. For each dataset we use 400 bones, 8 weights per vertex, and 6000 non-zero values for transformations, except the Louise and Carlos datasets, where 20000 non-zero values were required for high-quality results. 
...
MAE is the mean over all vertices of $|\matA - \matB\matC|$ and MXE $= max |\matA - \matB\matC|$ over all vertices.  The third metric attempts to measure noise introduced by optimization and quantization, and we use the average edge angular difference (EAD). This is calculated as the mean of $ | \beta_{A} - \beta_{BC} | $  over all edges, where $\beta$ represents the dihedral angle between an edge's adjacent faces.
