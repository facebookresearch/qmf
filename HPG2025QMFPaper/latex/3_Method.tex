...
\begin{equation}
  \vertexRestPose_i + \sum_{k} \bSWeight_k \Delta\vertexPos_{k,i},
\label {eq:ShapeCalculation}
\end{equation}
where $\vertexRestPose_i\in\reals^3$ is the $i$-th rest-pose vertex, $\Delta\vertexPos_{k,i}$  denotes the positional difference between the $i$-th vertex of the $k$-th blendshape and its corresponding vertex in the rest pose, and
...
\end{bmatrix}
\]
and write \Equation{eq:ShapeCalculation} in matrix form:
\begin{equation}
...
\begin{figure}
    ...
    \caption{QMF Results. Synthetic dataset: every second blendshape is "right eyebrow up" with a single "left eyebrow up." Target expression is "both eyebrows up." (left to right): original loss; new loss (wrinkles on right side only); new loss with wrinkle map $\wrinklesMap$ (wrinkles on both sides).}
...
To store the indices and quantized non-zero values of both matrices $\matB$ and $\matC$, we require $\numVertices + 3\numBS + \numNonzeroBC + \numNonzeroBC\numBitsPerNz/8$ bytes.
...
\end{algorithm}
\end{comment}
Compared to the  HSS representation of \citet{seo2011compression}, our method employs a much simpler runtime data structure. %, resulting in more straightforward code
Calculation of  \Equation{eq:MatShapeCalculation} with sparse representation of matrix $\matA$ requires two matrix multiplications and a vector addition:
\[
  \matBSWeight \matB \matC + \restPose.
\]
The result of the first calculation is a dense matrix $\matBSWeight \matB\in\reals^{3\times\numColumnB}$. We allocate the column-major matrix $\matWB$ to hold $3\times\numColumnB$ float numbers and initialize it with zeros.
