...
\end{comment}
To perform this multiplication efficiently we iterate over the array $\bSWeight$. For every non-zero weight value we need to iterate over 3 rows of the matrix $\matB$. If $\matB$ has non-zero values for a given weight, we perform multiplication and store the result in $\matWB$.
