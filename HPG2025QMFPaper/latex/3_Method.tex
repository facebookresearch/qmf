...
\end{bmatrix}
\]
Modern models often consist of hundreds to thousands of blendshapes and thus the storage of the matrix $\matA$ is an important consideration. The runtime speed of facial expression calculation is primarily determined by fetching elements of $\matA$ from memory. One of the main objectives of our method is to develop an efficient representation for $\matA$ to enhance the real-time performance of facial expression calculations while simultaneously reducing the memory footprint.
...
\begin{equation}
 \min_{\matB, \matC}( \|\matA - \matB \matC \| + \alpha \|\Laplacian \matB \matC\| )
 \label{eq:OldLoss}
\end{equation}
...
To address these issues, we propose to replace the $\alpha \|\Laplacian \matB \matC\|$ with the following term: %loss that avoids the problems of the Laplacian regularizer: $\newNorm(\matA - \approxMatA)$, where 
\begin{equation}
\alpha \|\Laplacian(\matA - \matB \matC ) \|
%\newNorm(\matM) = \| \matM \| + \alpha\|\Laplacian\matM\|
%\|\Laplacian(\approxMatA -\matA)\|
  \label{eq:NewLoss}
\end{equation}
%
The seemingly minor change of applying the Laplacian to $\matA - \matB \matC$ means that the original blendshapes (i.e. the exact reconstruction: $\matA = \matB \matC$) would now receive zero regularization penalty -- even in high-curvature areas, avoiding the oversmoothing issues of the previous approach.
...
\begin{figure}
  \parbox{0.3\linewidth}{
    \includegraphics[width=\linewidth]{NnzBoz.jpg}
    \subcaption{}
    \label{nonZeroDistribution}
  }
  \parbox{0.29\linewidth}{
    \includegraphics[width=\linewidth]{WrinclesMapBoz.jpg}
    \subcaption{}
    \label{wrinklesMap}
  }
  \parbox{0.3\linewidth}{
    \includegraphics[width=\linewidth]{NnzBozAfter.jpg}
    \subcaption{}
...
\begin{figure}
    ...
    \caption{Wrinkles density calculation and QMF results left to right: Result with original loss function, new loss (wrinkles on right side only), new loss and wrinkles map (wrinkles on both sides) }
...
Experimental results indicate that  $\numColumnB = 200$ (columns of $\matB$ and rows of $\matC$) is sufficient to achieve low errors. Because $\numColumnB \le 255$, we use an 8-bit representation to encode the column index for matrix $\matB$ and the row index for matrix $\matC$. Instead of storing an array of offsets, we store the number of non-zero values per row in matrix $\matB$ or per column in matrix $\matC$ and calculate offsets in order to calculate the next dot product during matrix multiplication. For example, see how \texttt{numValuesRow} is used in \Algorithm{alg:RuntimeCode}. 
...
\begin{figure}
    ...
    \includegraphics[width=\linewidth]{quantizationMetric.pdf}
...
\begin{equation*}
    V_Q = \text{round}((V - V_{min})/(V_{max} - V_{min}) \cdot (2^Q - 1))
\end{equation*}

We store $V_{min}$, $V_{max}$, and $Q$, and this allows us to dequantize via: 
\begin{equation*}    
    V = V_{min} + (V_Q * (V_{max} - V_{min}) )/(2^Q - 1)
\end{equation*}
...
\begin{figure}
    % TODO make better image
    \includegraphics[width=\linewidth]{lossHistory.pdf}
...
The final step of the algorithm is to multiply the small dense matrix $\matWB$ with $\matC$. We iterate over the columns of the matrix $\matC$, and for every non-zero value we will read a column in matrix $\matWB$. We add the result of multiplication to the value from the rest pose and store the result. The CPU source code is available in \Algorithm{alg:RuntimeCode}.
