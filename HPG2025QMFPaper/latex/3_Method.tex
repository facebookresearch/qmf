...
\begin{equation}
  \replace{\vertexPos_{0,i}}{\vertexRestPose_i} + \sum_{k} \bSWeight_k \Delta\vertexPos_{k,i},
\label {eq:ShapeCalculation}
\end{equation}
where $\replace{\vertexPos_{0,i}}{\vertexRestPose_i}\in\reals^3$ is the $i$-th rest-pose vertex, $\Delta\vertexPos_{k,i}$  denotes the positional difference between the $i$-th vertex of the $k$-th blendshape and its corresponding vertex in the rest pose, and
...
\end{bmatrix}
\]
\add{and write \Equation{eq:ShapeCalculation} in matrix form:
\begin{equation}
\restPose + \matBSWeight\matA,
\label {eq:MatShapeCalculation}
\end{equation}
where $\matBSWeight = (\identity \bSWeight_1, \dots, \identity \bSWeight_\numBS)$, $\identity$ is a $3\times3$ identity matrix and $\restPose\in\reals^{3\times\numVertices}$ are vertex positions of the rest pose.
}
...
We found that both Dem Bones and \csfb lose wrinkles at medium to high compression rates (see \Figure{fig:Teaser}). This is because these previous works apply a Laplacian regularizer to ensure the approximation error is smooth:
\begin{equation}
...
\end{equation}
%
The seemingly minor change of applying the Laplacian to $(\matA - \matB \matC)^\tran$ means that the original blendshapes (i.e., the exact reconstruction: $\matA = \matB \matC$) would now receive zero regularization penalty -- even in high-curvature areas, avoiding the oversmoothing issues of the previous approach. We can say that \Equation{eq:OldLoss} makes a \emph{smooth mesh} while \Equation{eq:NewLoss} makes a \emph{smooth error}.
...
\subref{wrinklesMap} wrinkles density $\wrinklesMap$;
...
\begin{figure}
    ...
    \caption{\replace{Wrinkle density calculation and QMF Results (left to right): result with original loss function; result with new loss (wrinkles on right side only); result with new loss and wrinkle map (wrinkles on both sides).}{QMF Results. Synthetic dataset: every second blendshape is "right eyebrow up" with a single "left eyebrow up." Target expression is "both eyebrows up." (left to right): original loss; new loss (wrinkles on right side only); new loss with wrinkle map $\wrinklesMap$ (wrinkles on both sides).}}
...
Experimental results indicate that  $\numColumnB = 200$ (columns of $\matB$ and rows of $\matC$) is sufficient to achieve low errors. Because $\numColumnB \le 255$, we use an 8-bit representation to encode the column index for matrix $\matB$ and the row index for matrix $\matC$. Instead of storing an array of offsets, we store the number of non-zero values per row in matrix $\matB$ or per column in matrix $\matC$ and compute offsets in order to calculate the next dot product during matrix multiplication. 
%For example, see how \texttt{numValuesRow} is used in \Algorithm{alg:RuntimeCode}. 
Consequently, to index both matrices, we need to store $\numVertices + 3\numBS + \numNonzeroBC$ bytes for $\numVertices$ vertices, $\numBS$ blendshapes, and $\numNonzeroBC$ non-zero values across $\matB$ and $\matC$.
%
%--- Quantization
\begin{figure}
...
After optimization is complete, we independently quantize the non-zero values for both matrices $\matB$ and $\matC$. For each of  $\matB$ and $\matC$, we find the minimum and maximum scalar values.  Then we use the following quantization formula for $\numBitsPerNz$ bits:
\begin{equation*}
  V_\numBitsPerNz = \text{round}((V - V_{\mathrm{min}})/(V_{\mathrm{max}} - V_{\mathrm{min}}) \cdot (2^\numBitsPerNz - 1))
\end{equation*}

We store $V_{\mathrm{min}}$, $V_{\mathrm{max}}$, and $\numBitsPerNz$, and this allows us to dequantize via: 
\begin{equation*}    
  V = V_{\mathrm{min}} + (V_\numBitsPerNz \cdot (V_{\mathrm{max}} - V_{\mathrm{min}}) )/(2^\numBitsPerNz - 1)
\end{equation*}

\add{To store the indices and quantized non-zero values of both matrices $\matB$ and $\matC$, we require $\numVertices + 3\numBS + \numNonzeroBC + \numNonzeroBC\numBitsPerNz/8$ bytes.}
...
\end{algorithm}
\end{comment}
\add{Compared to the  HSS representation of \citet{seo2011compression}, our method employs a much simpler runtime data structure.} %, resulting in more straightforward code
Calculation of  \replace{\Equation{eq:ShapeCalculation}}{\Equation{eq:MatShapeCalculation}} with sparse representation of matrix $\matA$ requires two matrix multiplications and a vector addition:
\[
  \matBSWeight \matB \matC + \restPose.
\]
\remove{where $\matBSWeight = (\identity \matBSWeight_1, \dots, \identity \matBSWeight_\numBS)$, $\identity$ is a $3\times3$ identity matrix and $\restPose\in\reals^{3\times\numVertices}$ are vertex positions of the rest pose.} The result of the first calculation is a dense matrix $\matBSWeight \matB\in\reals^{3\times\numColumnB}$. We allocate the column-major matrix $\matWB$ to hold $3\times\numColumnB$ float numbers and initialize it with zeros.
...
\end{comment}
To perform this multiplication efficiently we iterate over the array $\matBSWeight$. For every non-zero weight value we need to iterate over 3 rows of the matrix $\matB$. If $\matB$ has non-zero values for a given weight, we perform multiplication and store the result in $\matWB$.
...
Efficiently computing this reconstruction on the GPU requires some modification, including additional memory.  Notably, in order to compute $\matWB$, we precompute a per-nonzero-$\matB$-value lookup to see which blendshape weight to use, and whether the value corresponds to $x$, $y$, or $z$.  This data is packed into 16-bits per value in $\matB$, and allows us to compute $\matWB$ in parallel across non-zero $\matB$ values.  Additionally, in order to compute $\matWB \matC$, we precompute a 16- or 24-bit offset per vertex to assist with indexing into $\matC$ (with number of bits depending on how large $\matC$ is).  This enables us to run in parallel across vertices. \Algorithm{alg:CudaCode} shows the our kernel code.  We note that atomic addition in the $\matWB$ calculation does lead to some nondeterminism, but in practice errors are in the range of nanometers.
