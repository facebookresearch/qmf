...
def rig_control_remap(control_tensor):
    ...
    cheekPuff_L = torch.clamp(
        control_tensor[:, 2], min=0, max=1
    )  # linked with cheekSuck_L
    cheekPuff_R = torch.clamp(
        control_tensor[:, 3], min=0, max=1
    )  # linked with cheekSuck_R
    cheekRaiser_L = torch.clamp(control_tensor[:, 4], min=0, max=1)
    cheekRaiser_R = torch.clamp(control_tensor[:, 5], min=0, max=1)
    cheekSuck_L = (
        torch.clamp(control_tensor[:, 2], min=-1, max=0) * -1
    )  # linked with cheekPuff_L
    cheekSuck_R = (
        torch.clamp(control_tensor[:, 3], min=-1, max=0) * -1
    )  # linked with cheekPuff_R
    ...
    eyesClosed_L = torch.clamp(
        control_tensor[:, 10], min=0, max=1
    )  # linked with upperLidRaiser_L
    eyesClosed_R = torch.clamp(
        control_tensor[:, 11], min=0, max=1
    )  # linked with upperLidRaiser_R
    eyesLookDown_L = (
        torch.clamp(control_tensor[:, 12], min=-1, max=0) * -1
    )  # linked with eyesLookUp_L
    eyesLookDown_R = (
        torch.clamp(control_tensor[:, 13], min=-1, max=0) * -1
    )  # linked with eyesLookUp_R
    eyesLookLeft_L = (
        torch.clamp(control_tensor[:, 14], min=-1, max=0) * -1
    )  # linked with eyesLookRight_L
    eyesLookLeft_R = (
        torch.clamp(control_tensor[:, 15], min=-1, max=0) * -1
    )  # linked with eyesLookRight_R
    eyesLookRight_L = torch.clamp(
        control_tensor[:, 14], min=0, max=1
    )  # linked with eyesLookLeft_L
    eyesLookRight_R = torch.clamp(
        control_tensor[:, 15], min=0, max=1
    )  # linked with eyesLookLeft_R
    eyesLookUp_L = torch.clamp(
        control_tensor[:, 12], min=0, max=1
    )  # linked with eyesLookDown_L
    eyesLookUp_R = torch.clamp(
        control_tensor[:, 13], min=0, max=1
    )  # linked with eyesLookDown_R
    ...
    jawSidewaysLeft = (
        torch.clamp(control_tensor[:, 19], min=-1, max=0) * -1
    )  # linked with jawSidewaysRight
    jawSidewaysRight = torch.clamp(
        control_tensor[:, 19], min=0, max=1
    )  # linked with jawSidewaysLeft
    ...
    lipFunneler_LB = torch.clamp(
        control_tensor[:, 27], min=0, max=1
    )  # linked with lipSuck_LB
    lipFunneler_LT = torch.clamp(
        control_tensor[:, 28], min=0, max=1
    )  # linked with lipSuck_LT
    lipFunneler_RB = torch.clamp(
        control_tensor[:, 29], min=0, max=1
    )  # linked with lipSuck_RB
    lipFunneler_RT = torch.clamp(
        control_tensor[:, 30], min=0, max=1
    )  # linked with lipSuck_RT
    ...
    lipSuck_LB = (
        torch.clamp(control_tensor[:, 27], min=-1, max=0) * -1
    )  # linked with lipFunneler_LB
    lipSuck_LT = (
        torch.clamp(control_tensor[:, 28], min=-1, max=0) * -1
    )  # linked with lipFunneler_LT
    lipSuck_RB = (
        torch.clamp(control_tensor[:, 29], min=-1, max=0) * -1
    )  # linked with lipFunneler_RB
    lipSuck_RT = (
        torch.clamp(control_tensor[:, 30], min=-1, max=0) * -1
    )  # linked with lipFunneler_RT
    ...
    mouthLeft = torch.clamp(
        control_tensor[:, 45], min=0, max=1
    )  # linked with mouthRight
    mouthRight = (
        torch.clamp(control_tensor[:, 45], min=-1, max=0) * -1
    )  # linked with mouthLeft
    ...
    nostrilCompressor_L = (
        torch.clamp(control_tensor[:, 50], min=-1, max=0) * -1
    )  # linked with nostrilDilator_L
    nostrilCompressor_R = (
        torch.clamp(control_tensor[:, 51], min=-1, max=0) * -1
    )  # linked with nostrilDilator_R
    nostrilDilator_L = torch.clamp(
        control_tensor[:, 50], min=0, max=1
    )  # linked with nostrilCompressor_L
    nostrilDilator_R = torch.clamp(
        control_tensor[:, 51], min=0, max=1
    )  # linked with nostrilCompressor_R
    outerBrowRaiser_L = torch.clamp(control_tensor[:, 52], min=0, max=1)
    outerBrowRaiser_R = torch.clamp(control_tensor[:, 53], min=0, max=1)
    upperLidRaiser_L = (
        torch.clamp(control_tensor[:, 10], min=-1, max=0) * -1
    )  # linked with eyesClosed_L
    upperLidRaiser_R = (
        torch.clamp(control_tensor[:, 11], min=-1, max=0) * -1
    )  # linked with eyesClosed_R
...
def piecewise_linear(x, x1, x2, x3):
    return (x >= x1) * (x < x2) * ((x2 - x) / (x1 - x2) + 1.0) + (
        (x >= x2) * (x <= x3) * ((x - x2) / (x2 - x3) + 1.0)
    )


def rig_logic(control_tensor):
    rig_logic_tensor = torch.empty(control_tensor.shape[0], 287).to(
        control_tensor.device
    )
    rig_logic_tensor[:, :72] = control_tensor

    rig_logic_tensor[:, 72] = piecewise_linear(
        control_tensor[:, 24], x1=0.5, x2=0.75, x3=1.0
    )
    ...
    rig_logic_tensor[:, [75, 76, 92, 93, 100, 101]] = piecewise_linear(
        control_tensor[:, [4, 5, 32, 33, 46, 47]], x1=0.25, x2=0.5, x3=1.0
    )

    rig_logic_tensor[:, 87] = piecewise_linear(
        control_tensor[:, 24], x1=0.25, x2=0.5, x3=0.75
    )

    rig_logic_tensor[:, range(104, 111)] = piecewise_linear(
        control_tensor[:, [4, 5, 24, 32, 33, 46, 47]], x1=0.0, x2=0.25, x3=0.5
    )

    rig_logic_tensor[:, range(111, 115)] = piecewise_linear(
        control_tensor[:, [48, 49, 50, 51]], x1=0.0, x2=0.25, x3=1.0
    )

    rig_logic_tensor[:, range(115, 221)] = (
        control_tensor[
            :,
            [
                4,
    ...
    rig_logic_tensor[:, range(223, 229)] = (
        control_tensor[:, [22, 23, 66, 67, 12, 13]] * rtmp[:, [0, 1, 0, 1, 2, 3]]
    )
    rig_logic_tensor[:, range(229, 233)] = (
        rtmp[:, [2, 3, 2, 3]] * control_tensor[:, [22, 23, 66, 67]]
    )

    rig_logic_tensor[:, range(233, 277)] = (
        control_tensor[
            :,
            [
                12,
    ...
    rig_logic_tensor[:, [277, 278]] = (
        rtmp[:, [2, 3]] * control_tensor[:, [22, 23]] * rtmp[:, [0, 1]]
    )
    rig_logic_tensor[:, [279, 280]] = (
        rtmp[:, [2, 3]] * rtmp[:, [0, 1]] * control_tensor[:, [66, 67]]
    )
    rig_logic_tensor[:, range(281, 287)] = (
        control_tensor[:, [12, 13, 40, 40, 41, 41]]
        * control_tensor[:, [4, 5, 34, 35, 36, 37]]
        * control_tensor[:, [28, 29, 24, 24, 24, 24]]
        * control_tensor[:, [0, 1, 42, 43, 44, 45]]
    )
...
fixed_shapes_map_v2 = [
    [0, 1],
    [1.0, 1.0],  # 1, browLowerer
    [2, 3],
    [1.0, 1.0],  # 2, cheekPuff
    [6, 7],
    [1.0, 1.0],  # 3, chinRaiser
    [8, 9],
    [1.0, 1.0],  # 4, dimpler
    [10, 11],
    [1.0, 1.0],  # 5, eyesClosed
    [12, 13],
    [-1.0, -1.0],  # 6, eyesLookDown
    [14, 15],
    [-1.0, -1.0],  # 7, eyesLookLeft
    [14, 15],
    [1.0, 1.0],  # 8, eyesLookRight
    [12, 13],
    [1.0, 1.0],  # 9, eyesLookUp
    [16, 17],
    [1.0, 1.0],  # 10, innerBrowRaiser
    [18],
    [1.0],  # 11, jawDrop
    [21, 22],
    [1.0, 1.0],  # 12, lidTightener
    [23, 24],
    [1.0, 1.0],  # 13, lipCornerDepressor
    [25, 26],
    [1.0, 1.0],  # 14, lipCornerPuller
    [27, 28, 29, 30],
    [1.0, 1.0, 1.0, 1.0],  # 15, lipFunneler
    [33, 34],
    [1.0, 1.0],  # 16, lipPucker
    [39, 40],
    [1.0, 1.0],  # 17, lipStretcher
    [27, 28, 29, 30],
    [-1.0, -1.0, -1.0, -1.0],  # 18, lipSuck
    [41, 42],
    [1.0, 1.0],  # 19, lipTightener
    [43, 44],
    [1.0, 1.0],  # 20, lowerLipDepressor
    [45],
    [1.0],  # 21, mouthLeft
    [45],
    [-1.0],  # 22, mouthRight
    [46, 47],
    [1.0, 1.0],  # 23, nasolabialFurrow
    [48, 49],
    [1.0, 1.0],  # 24, noseWrinkler
    [52, 53],
    [1.0, 1.0],  # 25, outerBrowRaiser
    [10, 11],
    [-1.0, -1.0],  # 26, upperLidRaiser
    [54, 55],
    [1.0, 1.0],  # 27, upperLipRaiser
]
...
fixed_shapes_map_v1 = [
    ...
    [1.0, 1.0],  # 3, dimpler
    [10, 11],
    [1.0, 1.0],  # 4, eyesClosed
    [12, 13],
    [-1.0, -1.0],  # 5, eyesLookDown
    [14, 15],
    [-1.0, -1.0],  # 6, eyesLookLeft
    [14, 15],
    [1.0, 1.0],  # 7, eyesLookRight
    [12, 13],
    [1.0, 1.0],  # 8, eyesLookUp
    [16, 17],
    [1.0, 1.0],  # 9, innerBrowRaiser
    [21, 22],
    [1.0, 1.0],  # 10, lidTightener
    [23, 24],
    [1.0, 1.0],  # 11, lipCornerDepressor
    [25, 26],
    [1.0, 1.0],  # 12, lipCornerPuller
    [27, 28, 29, 30],
    [1.0, 1.0, 1.0, 1.0],  # 13, lipFunneler
    [33, 34],
    [1.0, 1.0],  # 14, lipPucker
    [39, 40],
    [1.0, 1.0],  # 15, lipStretcher
    [27, 28, 29, 30],
    [-1.0, -1.0, -1.0, -1.0],  # 16, lipSuck
    [41, 42],
    [1.0, 1.0],  # 17, lipTightener
    [43, 44],
    [1.0, 1.0],  # 18, lowerLipDepressor
    [45],
    [1.0],  # 19, mouthLeft
    [45],
    [-1.0],  # 20, mouthRight
    [46, 47],
    [1.0, 1.0],  # 21, nasolabialFurrow
    [52, 53],
    [1.0, 1.0],  # 22, outerBrowRaiser
    [10, 11],
    [-1.0, -1.0],  # 23, upperLidRaiser
    [54, 55],
    [1.0, 1.0],  # 24, upperLipRaiser
]
...
def compute_rig_logic(control_tensor, inbetween_dict, corrective_dict):
    ...
    rig_logic_size = (
        control_size + inbetween_dict["num_indices"] + corrective_dict["num_indices"]
    )

    rig_logic_tensor = torch.empty(control_tensor.shape[0], rig_logic_size).to(
        control_tensor.device
    )
    ...
    for inbetween_values in inbetween_dict.keys():
        if inbetween_values == "num_indices":
            continue
        ...
        # vectorized compute
        rig_logic_tensor[:, driver_range_mask] = piecewise_linear(
            control_tensor[:, driven_range_mask],
    ...
    for num_mults in corrective_dict.keys():
        ...
        for j in range(1, num_mults):
            corrective_value = (
                corrective_value * tmp_tensor[:, corrective_dict[num_mults][j]]
            )
