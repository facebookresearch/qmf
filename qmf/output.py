...
def loadAnimWeights(p, npb):
    if p.model == "carlos":
        return np.load(f"{geometry.dataPath}/carlos_anim.npz")["weights"]
    elif p.model == "dragon":
        return np.load(f"{geometry.dataPath}/dragon_anim.npz")["weights"]
    modelsWithRig = ("aura", "boz", "jupiter", "proteus")
    if p.model not in modelsWithRig:
        npb = geometry.loadModel("jupiter")  # model with big number of blendspahes
