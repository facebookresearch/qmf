...
1. Script `sparseFactorization.py` loads geometry data from numpy .npz files. These files should be downloaded into
   dataPath folder (change dataPath variable)
   ```py
    geometry.dataPath = "<path to folder with .npz files>"
   ```
...
2. To run script edit  `runModel()` function
    ...
    and run script
    ```
    $ python3.12 sparseFactorization.py
    ```
